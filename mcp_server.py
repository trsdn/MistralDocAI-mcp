...
async def list_tools() -> List[Tool]:
    ...
    return [
        ...
        Tool(
            ...
            description="Batch process all supported documents in a directory",
            inputSchema={
                "type": "object",
                "properties": {
                    "input_directory": {
                    ...
                    "output_directory": {
                        "type": "string",
                        "description": "Path to the directory where markdown files will be saved"
                    }
                },
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    try:
        if name == "process_document":
            ...
            return await handle_process_directory(arguments)
...
async def handle_process_directory(arguments: Dict[str, Any]) -> List[TextContent]:
    """Handle directory processing requests"""
    ...
    if not input_dir.is_dir():
        return [TextContent(type="text", text=f"Input path is not a directory: {input_dir}")]
    
    try:
        success_count, failed_count = processor.process_directory(input_dir, output_dir)
        
        result_text = f"Directory processing completed:\n"
        result_text += f"- Successful conversions: {success_count}\n"
        result_text += f"- Failed conversions: {failed_count}\n"
        result_text += f"- Output directory: {output_dir}"
        
        return [TextContent(type="text", text=result_text)]
        
    except Exception as e:
        return [TextContent(type="text", text=f"Directory processing failed: {str(e)}")]
